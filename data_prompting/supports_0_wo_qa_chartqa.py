...
def create_zero_shot_supports_prompt_wo_QA(image, annotations):
    ''' Create prompt for zero-shot supports generation without Q&A. '''
    prompt = f"""
    You are a helpful assistant designed to output JSON.

    You will be provided with an image of a chart and a JSON file with annotations related to the chart, delimited by triple single quotes.
...
def process_subdirectory(base_dir, subdirectory, model):
    ...
    output_file = os.path.join(sub_dir, "results.json")
