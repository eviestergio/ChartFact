...
def create_few_shot_supports_prompt_wo_QA(image, annotations):
    """ Creates a few-shot prompt to generate a 'supports' claim with an explanation using image input and annotations without a Q&A pair. """
    ...
    Task: Using the two input-output examples below delimited by angle brackets, generate a claim that supports the information in the chart and annotations, and provide an explanation.

    Process for generating a 'supports' claim and explanation:
        1. Using the image and annotations, develop a claim that is supported by the data in the chart.
...
def process_subdirectory(base_dir, subdirectory, model):
    ...
    output_file = os.path.join(sub_dir, "results_supports_few_wo_qa.json")
